...
function explainPlan(plan: {
  ...
  if (plan.buySideNotionalUsd !== undefined && plan.sellSideNotionalUsd !== undefined) {
    const ratio =
      plan.sellSideNotionalUsd > 0
        ? plan.buySideNotionalUsd / plan.sellSideNotionalUsd
        : 1;
    let tilt = "neutral";
    if (ratio < 0.95) tilt = "sell-tilted (collecting positive funding)";
    else if (ratio > 1.05) tilt = "buy-tilted (collecting negative funding)";
