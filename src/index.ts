...
function main(): void {
  ...
  switch (cmd) {
    ...
    case "explain": {
      ...
      if (!raw.trim()) {
        throw new Error(
          "explain needs a GridPlan JSON. Pipe the output of `liqgrid plan` into it."
        );
      }
...
function explainPlan(plan: {
  ...
  for (const field of required) {
    if ((plan as Record<string, unknown>)[field] === undefined) {
      throw new Error(
        `explain: input is not a GridPlan — missing field \`${field}\`. Pipe the output of \`liqgrid plan\` into \`liqgrid explain\`.`
      );
    }
  }
