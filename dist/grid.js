...
// ---------------------------------------------------------------------------
export function realizedVolatilityDaily(candles) {
    if (candles.length < 2)
        return 0;
    const logReturns = [];
...
// ---------------------------------------------------------------------------
export function chooseGridCount(rangeLow, rangeHigh, dailyVol, riskProfile, tickSize) {
...
// ---------------------------------------------------------------------------
function computeStopLoss(rangeLow, rangeHigh, markPrice, riskProfile, totalNotionalUsd, leverage) {
    ...
    const side = markPrice <= mid ? "long" : "short";
    let triggerPrice;
    if (side === "long") {
...
// ---------------------------------------------------------------------------
function estimateFillsPerDay(dailyVol, gridCount, rangeLow, rangeHigh) {
    const rangePct = (rangeHigh - rangeLow) / ((rangeHigh + rangeLow) / 2);
    if (rangePct === 0)
        return 0;
...
// on this estimate.
// ---------------------------------------------------------------------------
function liquidationDistancePct(leverage) {
...
// ---------------------------------------------------------------------------
export function computeGridPlan(input) {
    const warnings = [];
    ...
    if (!input || typeof input !== "object") {
        ...
        if (!["conservative", "balanced", "aggressive"].includes(input.riskProfile)) {
        ...
        if (!input.marketMeta || typeof input.marketMeta !== "object") {
            ...
            for (const f of ["tickSize", "minOrderSizeUsd", "markPrice", "maxLeverage"]) {
                if (typeof input.marketMeta[f] !== "number") {
        ...
        if (!Array.isArray(input.candles)) {
            ...
            structuralProblems.push(`candles array has ${input.candles.length} entries; need at least 2 for volatility estimate, 24+ recommended`);
        }
        ...
        if (typeof input.rangeLow === "number" &&
            typeof input.rangeHigh === "number" &&
            input.rangeLow >= input.rangeHigh) {
            structuralProblems.push(`\`rangeLow\` (${input.rangeLow}) must be strictly less than \`rangeHigh\` (${input.rangeHigh})`);
        }
        ...
        if (input.marketMeta && typeof input.marketMeta === "object") {
            const mm = input.marketMeta;
            for (const f of ["tickSize", "markPrice", "maxLeverage"]) {
                if (typeof mm[f] === "number" && mm[f] <= 0) {
    ...
    if (structuralProblems.length > 0) {
        ...
        return {
            ...
            marginRequiredUsd: 0,
            leverage: 0,
            riskProfile: input?.riskProfile === "balanced" || input?.riskProfile === "aggressive"
                ? input.riskProfile
                : "conservative",
    ...
    if (input.totalNotionalUsd > CAPS.MAX_NOTIONAL_USD) {
        warnings.push(`total_notional_usd (${input.totalNotionalUsd}) exceeds cap (${CAPS.MAX_NOTIONAL_USD}); clamped`);
    }
    if (input.leverage > CAPS.MAX_LEVERAGE) {
        warnings.push(`leverage (${input.leverage}) exceeds cap (${CAPS.MAX_LEVERAGE}); clamped`);
    }
    if (input.leverage > input.marketMeta.maxLeverage) {
        warnings.push(`leverage (${input.leverage}) exceeds market max (${input.marketMeta.maxLeverage}); clamped`);
    }
    ...
    if (input.marketMeta.markPrice < input.rangeLow - rangeHalfWidth * 0.25 ||
        input.marketMeta.markPrice > input.rangeHigh + rangeHalfWidth * 0.25) {
        warnings.push(`REFUSE: mark price (${input.marketMeta.markPrice}) is significantly outside [${input.rangeLow}, ${input.rangeHigh}] — the user's range premise is likely stale or wrong; do not execute`);
    }
    else if (input.marketMeta.markPrice < input.rangeLow ||
        input.marketMeta.markPrice > input.rangeHigh) {
        warnings.push(`mark price (${input.marketMeta.markPrice}) is outside [${input.rangeLow}, ${input.rangeHigh}] — grid will open asymmetric; recheck range`);
    }
    ...
    if (input.candles.length > 0) {
        ...
        if (lastClose > 0) {
            ...
            if (drift > 0.03) {
                warnings.push(`mark price (${input.marketMeta.markPrice}) drifted ${(drift * 100).toFixed(1)}% from last candle close (${lastClose}); volatility estimate may be stale — consider refetching candles`);
            }
        }
    }
    ...
    if (input.candles.length >= 2) {
        for (let i = 1; i < input.candles.length; i++) {
            ...
            if (dt > 2 * 3_600_000) {
                warnings.push(`candle series has a ${(dt / 3_600_000).toFixed(1)}h gap at index ${i}; realized-volatility estimate may be low — consider refetching a contiguous window`);
                break; // one warning is enough; don't spam
            }
        }
    }
    const notional = Math.min(input.totalNotionalUsd, CAPS.MAX_NOTIONAL_USD);
    let leverage = Math.min(input.leverage, CAPS.MAX_LEVERAGE, input.marketMeta.maxLeverage);
    ...
    if (input.riskProfile === "conservative" && leverage > 5) {
        warnings.push(`conservative profile combined with ${leverage}× leverage auto-downshifted to 5× (use 'balanced' or 'aggressive' if you want higher leverage)`);
        leverage = 5;
    }
    ...
    let levels = buildLevels(input.rangeLow, input.rangeHigh, actualGridCount, input.marketMeta.markPrice, notional, input.marketMeta.tickSize, leverage, vol, fundingBias);
    const minOrder = input.marketMeta.minOrderSizeUsd;
    if (minOrder > 0) {
        while (actualGridCount > CAPS.MIN_GRID_COUNT &&
            levels.length > 0 &&
            Math.min(...levels.map((l) => l.sizeUsd * leverage)) < minOrder) {
            actualGridCount -= 1;
            levels = buildLevels(input.rangeLow, input.rangeHigh, actualGridCount, input.marketMeta.markPrice, notional, input.marketMeta.tickSize, leverage, vol, fundingBias);
        }
        if (actualGridCount < gridCount) {
            warnings.push(`gridCount auto-reduced from ${gridCount} to ${actualGridCount} so every rung clears market min order ($${minOrder}). To restore density, increase totalNotionalUsd or leverage.`);
        }
        ...
        if (levels.length > 0 &&
            Math.min(...levels.map((l) => l.sizeUsd * leverage)) < minOrder) {
            levels = buildLevels(input.rangeLow, input.rangeHigh, actualGridCount, input.marketMeta.markPrice, notional, input.marketMeta.tickSize, leverage, 0, // sigmaDaily=0 → uniform sizing fallback
            fundingBias // keep funding bias even in fallback
            );
            warnings.push(`notional ($${notional}) too small for concentrated-liquidity sizing at min order $${minOrder} — fell back to uniform per-rung sizing. Consider increasing totalNotionalUsd to ≥ $${Math.ceil(minOrder * actualGridCount * 2.5 / leverage)} for full concentrated-liquidity behavior.`);
        }
    }
    ...
    if (maxLossPct > CAPS.MAX_LOSS_PCT_OF_NOTIONAL) {
        warnings.push(`stop-loss would allow loss of ${(maxLossPct * 100).toFixed(1)}% > ${(CAPS.MAX_LOSS_PCT_OF_NOTIONAL * 100).toFixed(0)}% cap; widen range or tighten risk_profile`);
    }
    ...
    if (levels.length > 0) {
        ...
        for (const lvl of levels) {
            ...
            if (notionalAtLevel < minLevelNotional)
                minLevelNotional = notionalAtLevel;
        }
        if (minLevelNotional < input.marketMeta.minOrderSizeUsd) {
            warnings.push(`smallest per-level notional ${minLevelNotional.toFixed(2)} < market min ${input.marketMeta.minOrderSizeUsd}; reduce grid_count or increase total_notional_usd`);
        }
        // Defensive secondary check: if any level's sizeCoin rounds to 0 after
        // 8-decimal truncation, that level cannot be placed.
        for (const lvl of levels) {
            if (lvl.sizeCoin <= 0) {
                warnings.push(`level ${lvl.index} at price ${lvl.price} has sizeCoin rounded to 0 (notional too small for this price); reduce grid_count or increase total_notional_usd`);
                break;
            }
        }
    }
    ...
    const sellSideNotionalUsd = levels
        ...
        .reduce((acc, l) => acc + l.sizeUsd, 0);
    const rangeWidthPct = input.marketMeta.markPrice > 0
    ...
    const fillsPerDay = estimateFillsPerDay(vol, levels.length, input.rangeLow, input.rangeHigh);
...
export function runBacktest(input) {
    ...
    const btFeeRateMaker = typeof input.marketMeta.feeRateMaker === "number" && input.marketMeta.feeRateMaker >= 0
        ? input.marketMeta.feeRateMaker
        : 0.00015;
    let feesPaid = 0;
